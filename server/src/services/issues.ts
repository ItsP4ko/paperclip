  ...
  return {
    ...
    remove: (id: string) =>
      db.transaction(async (tx) => {
        ...
        // Delete dependent rows that lack ON DELETE CASCADE before removing issues
        await tx.delete(issueComments).where(inArray(issueComments.issueId, allIds));
        await tx.delete(issueInboxArchives).where(inArray(issueInboxArchives.issueId, allIds));
        await tx.delete(issueReadStates).where(inArray(issueReadStates.issueId, allIds));
        await tx.delete(feedbackVotes).where(inArray(feedbackVotes.issueId, allIds));
        await tx.update(financeEvents).set({ issueId: null }).where(inArray(financeEvents.issueId, allIds));
        await tx.update(costEvents).set({ issueId: null }).where(inArray(costEvents.issueId, allIds));
